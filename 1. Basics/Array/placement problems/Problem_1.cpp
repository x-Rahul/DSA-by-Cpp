...
Sarasvati has an array of N non-negative integers. The j-th integer of the array is
Ai.
...
contains N integers. The j-th integer is Ai.
...
{   
    ...
    for (int i = 1; i <= T; i++)
    {
        ...
        int pd = arr[1]-arr[0], curr = 2, ans = 2, j=2;
        while (j < N)
        {
            if(pd == arr[j] - arr[j-1]){
                curr++;
        ...
        cout<<ans<<endl<<"next test case"<<endl;
    }
    return 0;
}
